...
@Component
public class JdbcPhoneDaoBahtinova implements PhoneDaoBahtinova {
    ...
    private static final String SQL_SAVE_QUERY = "insert into phones values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    @Override
    public Optional<PhoneBahtinova> get(final Long key) {
       try {
           PhoneBahtinova phone = (PhoneBahtinova) jdbcTemplate.queryForObject(SQL_GET_QUERY, new Object[]{key}, new BeanPropertyRowMapper(PhoneBahtinova.class));
    ...
    @Transactional
    public void save(final PhoneBahtinova phone) {
        jdbcTemplate.update(SQL_SAVE_QUERY, new Object[]{phone.getId(), phone.getBrand(), phone.getModel(), phone.getPrice(), phone.getDisplaySizeInches(),
    ...
    @Override
    public List<PhoneBahtinova> findAll(String order, int offset, int limit) {
        String orderedQuery = String.format(SQL_FINDALL_QUERY, order);
        return jdbcTemplate.query(orderedQuery, new Object[]{limit, offset}, new BeanPropertyRowMapper(PhoneBahtinova.class));
    }

    @Override
    public List<PhoneBahtinova> search(String like, String order, int offset, int limit){
        String sqlLike = "%%" + like + "%%";
        String orderedQuery = String.format(SQL_SEARCH_QUERY, order);
        return jdbcTemplate.query(orderedQuery, new Object[]{sqlLike, limit, offset}, new BeanPropertyRowMapper(PhoneBahtinova.class));
    }

    @Override
    public List<ColorBahtinova> getPhoneColors(final Long key){
        return jdbcTemplate.query(SQL_GETCOLORS_QUERY, new Object[]{key}, (ResultSet rs, int rowNumber) -> {
                ColorBahtinova color = new ColorBahtinova();
    ...
    @Transactional
    public void savePhoneColors(final Long key, final Set<ColorBahtinova> colorSet){
        Iterator<ColorBahtinova> iter = colorSet.iterator();
        while(iter.hasNext()){
    ...
    @Override
    public Optional<StockBahtinova> getStock(final Long key){
        try{
            StockBahtinova stock = jdbcTemplate.queryForObject(SQL_CHECKSTOCK_QUERY, new Object[]{key}, new BeanPropertyRowMapper<>(StockBahtinova.class));
